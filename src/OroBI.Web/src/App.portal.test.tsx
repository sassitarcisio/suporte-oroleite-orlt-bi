...
describe('Seller portal', () => {
  ...
  it('ends the session after changing password without retaining secrets', async () => {
    ...
    await screen.findByRole('heading', { name: 'Meu desempenho' })
    fireEvent.click(screen.getByRole('button', { name: 'Mais' }))
    fireEvent.click(within(screen.getByRole('dialog', { name: 'Mais módulos' })).getByRole('button', { name: 'Perfil' }))
  ...
  it('displays the server ticket and purchased quantity on customer detail', async () => {
    ...
    render(<App />)
    await screen.findByRole('heading', { name: 'Meu desempenho' })
    fireEvent.click(within(screen.getByRole('navigation', { name: 'Navegação rápida' })).getByRole('button', { name: 'Clientes' }))
  ...
  it('allows local signout when the identity request fails', async () => {
    ...
    render(<App />)
    expect(await screen.findByRole('alert')).toHaveTextContent('Não foi possível validar seu acesso. Verifique sua conexão e tente novamente.')
    fireEvent.click(screen.getByRole('button', { name: 'Sair' }))
    expect(await screen.findByRole('button', { name: /ENTRAR/i })).toBeVisible()
    expect(sessionStorage.getItem('orobi.access-token')).toBeNull()
  })
