...
export default function App() {
  ...
  if (!token) return <main className="shell login-shell"><section className="login-card shadow-lg"><div className="brand"><img className="brand-logo" src="/logoOroleite.png" alt="Oroleite Distribuidora" /></div><div className="login-copy"><p className="eyebrow"><i className="fa-solid fa-lock" aria-hidden="true" /> ACESSO RESTRITO</p><h1>Entre no centro<br /><em>de resultados.</em></h1><p>Leitura clara para decisoes que movem a operacao.</p></div><form onSubmit={login}><label>E-MAIL<input type="email" required value={email} onChange={event => setEmail(event.target.value)} /></label><label>SENHA<input type="password" required value={password} onChange={event => setPassword(event.target.value)} /></label><button className="btn btn-dark" type="submit">Entrar <i className="fa-solid fa-arrow-right" aria-hidden="true" /></button></form>{state === 'error' && <p className="notice error">Credenciais invalidas ou API indisponivel.</p>}</section></main>
