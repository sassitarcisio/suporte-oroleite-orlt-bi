...
it('shows ordinary login without an expiry warning or Bearer remember checkbox when no cookie exists', async () => {
  ...
  expect(screen.getByText(/acesso permanece.*8 horas/)).toBeVisible()
  expectCookieCalls()
})

it('accepts cookie login without an accessToken and never serializes its local generation or password', async () => {
  let signedIn = false
...
it('preserves self-registration and uses the corporate request headers for the public registration endpoint', async () => {
  vi.stubGlobal('fetch', vi.fn(async (input: RequestInfo | URL) => String(input).endsWith('/auth/register') ? json({ message: 'Aguarde a aprovação do administrador.' }, 202) : json({}, 401)))
  render(<App />)
  await screen.findByRole('button', { name: 'Entrar' })
  fireEvent.click(screen.getByRole('button', { name: 'Criar minha conta' }))
  fireEvent.change(screen.getByLabelText('Nome completo'), { target: { value: 'Ana Pessoa' } })
  fireEvent.change(screen.getByLabelText('E-mail para cadastro'), { target: { value: 'ana@example.invalid' } })
  for (const label of ['Crie sua senha', 'Confirme sua senha']) fireEvent.change(screen.getByLabelText(label), { target: { value: 'Personal-123!' } })
  fireEvent.click(screen.getByRole('button', { name: 'Solicitar cadastro' }))
  expect(await screen.findByRole('status')).toHaveTextContent('Aguarde a aprovação')
  expectCookieCalls()
})
