...
export type PortalSale = { id: string; date: string; documentNumber: string; movementType: string; customerCode: string; customerName: string; productName: string; brand: string; quantity: number; totalValue: number; physicalTrades?: number | null; tradeToSalesPercent?: number | null }
...
export type PortalRanking = { items: Array<{ label: string; productCode?: string | null; grossSales: number; netRevenue: number; quantity: number; movementCount: number; customerCount: number | null; revenueSharePercent: number | null }>; totalCount: number; hasMore: boolean }
...
export type PortalPpp = { year: number; month: number; available: boolean; isApproved?: boolean; unavailableReason: string | null; achievementPercent: number | null; award: number | null; segments: Array<{ segment: string; customerCount: number | null; itemsPerSegment: number; groupsPlaced: number | null; achievementPercent: number | null }> }
