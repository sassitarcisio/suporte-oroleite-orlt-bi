  ...
  return <section className="closing-statement supervisor-statement" aria-label="Demonstrativo de Deivid Mannes">
    ...
    {ready && summary && <>
      <section className="statement-totals" aria-label="Resumo da remuneração" data-testid="closing-financial-summary">
        <article><h2><i className="card-label-icon fa-solid fa-wallet" aria-hidden="true" /> Salário-base</h2><strong>{money(summary.compensation.baseSalary)}</strong><p>Remuneração fixa</p></article>
        <article><h2><i className="card-label-icon fa-solid fa-calculator" aria-hidden="true" /> Comissões</h2><strong>{money(summary.compensation.commission)}</strong><p>Própria + equipe + redes</p></article>
        <article><h2><i className="card-label-icon fa-solid fa-trophy" aria-hidden="true" /> Prêmios no período</h2><strong>{money(summary.totalAwards)}</strong><p>Equipe + troca geral</p></article>
        <article className="statement-total"><h2><i className="card-label-icon fa-solid fa-money-check-dollar" aria-hidden="true" /> Total previsto</h2><strong>{money(summary.total)}</strong><p>Salário + comissões + prêmios</p></article>
      </section>
      {detail ? <>
        <section className="statement-primary" aria-label="Comissões por operação">
          <article className="statement-panel"><h2><i className="card-label-icon fa-solid fa-percent" aria-hidden="true" /> Vendas próprias · 1%</h2><div className="statement-panel-body"><p className="statement-label">Comissão própria</p><strong className="statement-amount">{money(detail.ownCommission)}</strong><p>1% sobre as vendas próprias sem bonificações.</p></div></article>
          <article className="statement-panel"><h2><i className="card-label-icon fa-solid fa-percent" aria-hidden="true" /> Equipe comercial · 0,15%</h2><div className="statement-panel-body"><p className="statement-label">Comissão da equipe</p><strong className="statement-amount">{money(detail.teamCommission)}</strong><p>0,15% sobre as vendas dos sete vendedores da equipe.</p></div></article>
          <article className="statement-panel"><h2><i className="card-label-icon fa-solid fa-percent" aria-hidden="true" /> Redes · 0,15%</h2><div className="statement-panel-body"><p className="statement-label">Comissão Bistek e Giassi</p><strong className="statement-amount">{money(detail.networkCommission)}</strong><p>0,15% sobre as vendas das redes, sem a Operação Bauducco.</p></div></article>
        </section>
        <section className="statement-table-panel"><h2><i className="card-label-icon fa-solid fa-arrow-right-arrow-left" aria-hidden="true" /> Resumo de vendas e trocas</h2><div className="statement-table-scroll"><table aria-label="Vendas e trocas por operação"><thead><tr><th>Operação</th><th>Venda líquida</th><th>Trocas</th><th>Devoluções de troca</th><th>Total de trocas</th><th>% de trocas</th></tr></thead><tbody>{detail.operations.map(operation => <tr key={operation.key} className={operation.key === 'total' ? 'statement-consolidated' : undefined}><th scope="row">{operation.label}</th><td>{money(operation.revenue)}</td><td>{money(operation.trade)}</td><td>{money(operation.tradeReturns)}</td><td>{money(operation.totalTrades)}</td><td>{percent(operation.tradePercent)}</td></tr>)}</tbody></table></div><p className="statement-table-note">Venda líquida sem bonificações. O total consolidado reúne vendas próprias, equipe e redes sem repetir movimentos presentes em mais de uma operação.</p></section>
        <div className="statement-awards">
          <section className="statement-panel" aria-label="Critérios da média da equipe"><h2><i className="card-label-icon fa-solid fa-trophy" aria-hidden="true" /> Prêmio médio da equipe</h2><div className="statement-panel-body"><p className="statement-label">Média deste demonstrativo</p><strong className="statement-amount">{money(detail.teamAverageAward)}</strong><p>Divisão pelos sete vendedores. Paulo participa das vendas e do divisor; seu prêmio exibido é zero porque ele não integra as linhas da folha.</p><dl className="statement-fields"><div><dt>Média aplicada na folha de pagamento</dt><dd>{money(detail.payrollTeamAverageAward)}</dd></div></dl><p>A folha considera os sete prêmios calculados, incluindo o prêmio de Paulo. Por isso, as médias podem ser diferentes.</p></div></section>
          <section className="statement-panel statement-trade" aria-label="Prêmio por troca geral"><h2><i className="card-label-icon fa-solid fa-trophy" aria-hidden="true" /> Prêmio por troca geral</h2><div className="statement-panel-body"><p className="statement-label">Trocas sobre a base consolidada</p><strong className="statement-amount">{percent(summary.monthly.tradePercent)}</strong><dl className="statement-fields"><div><dt>Total de trocas</dt><dd>{money(summary.monthly.tradeValue)}</dd></div><div><dt>Prêmio</dt><dd>{money(summary.tradeAward)}</dd></div></dl><p>Até 1,25%: R$ 5.000; até 1,75%: R$ 3.000; até 2,25%: R$ 2.000; acima de 2,25%: sem prêmio. Percentual arredondado a duas casas para aplicação das faixas. O prêmio exige base líquida positiva.</p></div></section>
        </div>
        <section className="statement-table-panel"><h2><i className="card-label-icon fa-solid fa-trophy" aria-hidden="true" /> Detalhamento dos prêmios da equipe</h2><div className="statement-table-scroll"><table aria-label="Prêmios da equipe"><thead><tr><th>Vendedor</th><th>Venda líquida</th><th>Trocas</th><th>Devoluções de troca</th><th>Total de trocas</th><th>% de trocas</th><th>PPP Nestlé</th><th>Prêmio metas</th><th>Prêmio total</th></tr></thead><tbody>{detail.team.map(member => <tr key={member.seller}><th scope="row">{member.seller}{!member.includedInPayroll && <small>Fora das linhas da folha</small>}</th><td>{money(member.sales.revenue)}</td><td>{money(member.sales.trade)}</td><td>{money(member.sales.tradeReturns)}</td><td>{money(member.sales.totalTrades)}</td><td>{percent(member.sales.tradePercent)}</td><td>{money(member.pppAward)}</td><td>{money(member.goalAward)}</td><td className="statement-row-total">{money(member.totalAward)}</td></tr>)}</tbody></table></div></section>
