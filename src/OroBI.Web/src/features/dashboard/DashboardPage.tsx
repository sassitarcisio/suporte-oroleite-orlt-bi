...
export function DashboardPage({ summary, filters, options, details, sellers, state, onFiltersChange, onSubmit, onClear }: DashboardPageProps) {
  ...
  return <section className="dashboard-layout">
    ...
    {summary && state === 'ready' && summary.movementCount > 0 && <section className="dashboard-metrics" data-testid="dashboard-metrics">
      ...
      <article className="dashboard-kpi negative"><p><i className="fa-solid fa-arrow-trend-down" aria-hidden="true" /> Movimentos negativos</p><strong>{money(summary.negativeMovements)}</strong><span>Devolucoes, trocas e descontos</span></article>
      <article className="dashboard-kpi petrol"><p><i className="fa-solid fa-chart-line" aria-hidden="true" /> Resultado liquido</p><strong>{money(summary.netResult)}</strong><span>Soma de todos os movimentos</span></article>
